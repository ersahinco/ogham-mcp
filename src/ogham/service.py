...
def search_memories_enriched(
    ...
    def _do_hybrid_search(q_text, q_emb, prof, lim, t, s):
        ...
        if query_sparse is not None:
            return hybrid_search_memories_sparse(
                q_text, q_emb, query_sparse, prof, lim, t, s
            )
    ...
    if is_ordering_query(query):
        results = _do_hybrid_search(
            query, embedding, profile, elastic_limit * 5, tags, source,
        )
        if results:
    ...
    if graph_depth > 0:
        ...
        results = _do_hybrid_search(
            query, embedding, profile, fetch_limit, tags, source,
        )
