...
-- (dim-agnostic -- works for any EMBEDDING_DIM) rather than halfvec. The
-- halfvec variant was 2x smaller in memory but required a hardcoded dim
-- in the cast, which broke when operators changed EMBEDDING_DIM.
-- Operators running at >100K rows who want halfvec memory savings back:
-- run migration 021_dim_aware_halfvec.sql with SET ogham.rebuild_hnsw='on'.
